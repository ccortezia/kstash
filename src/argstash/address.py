...
# - Example: mem://my-namespace/my-variable
GENERAL_ADDRESS_RE = re.compile(rf"^(?P<schema>mem)://{NS_RE}/{NAME_RE}$")
...
INLINE_ADDRESS_RE = re.compile(
    rf"^(?P<schema>inline)://{NS_RE}/{NAME_RE}/{ARGDATA_RE}$"
)
...
class Address:
    ...
    format: str = field(init=False, default="{schema}://{namespace}/{name}")
